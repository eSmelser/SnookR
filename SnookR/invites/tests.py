...
class TeamInvitesTestCase(SeleniumTestCase):
    def setUp(self):
        ...
        captain = User.objects.create_user(username='evan', password='password', first_name='evan', last_name='smelser')
        captain.user_permissions.add(permission)
        self.client.login(username='evan', password='password')
        self.user = User.objects.create_user(username='user', password='password', first_name='john', last_name='doe')
